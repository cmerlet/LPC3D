...
16th line: name of the file with the frequencies if direct/indirect before (> namefreq), 1st column should be in anstroms
17th line: number of frequencies if direct/indirect before (> Nfreq)
*/ 

/************************* Porous *************************/
/* THIS PART WILL BE MODIFIED WHEN A BETTER MODEL FOR THE CHEMICAL SHIFTS WILL BE DEFINED */
/* 
13th line: name of the file with the free energies (> nameeng), 1st column should be in anstroms, 2nd column should be in kJ/mol 
14th line: number of free energies values (> Neng)
15th line: do we use the values directly or do we use Xing fit (Carbon, 77, 1132, 2014)? (> poreshift)
16th line: name of the file with the frequencies if direct before (> namefreq), 1st column should be in anstroms
17th line: number of frequencies if direct before (> Nfreq)
*/ 
...
char namepsd[50],namesurf[50],nameecho[50];
...
double lattvacf,lattvacfx,lattvacfy,lattvacfz,v0;
double meanpsd,stdpsd,meansurf,stdsurf,meanEa,stdEa,int_tau0,int_tau;
double *area,*X,*Y,*Z;
...
int main(void)
{
 ...
 if(strcmp(simtype,"coarse")==0&&Enbar_mode==1)
   {
    equilibration();
   }
...
void read_porous()
{
 ...
 char ligne[L];
 int i,j,k,ibis,jbis,kbis,l,ener,idist,jdist,kdist,zcount,ibin;
 double denstot,part_func,dx,dy,dz,Lx,Ly,Lz,rmax,dist,distmin,distsurf;
 double meanengz,meanfreqz,meandensz,dr,distcarbon;
 double *distdenscarbon,*distdenssurf;
 ...
 distdenssurf=dvector(Nz+1);
 for(k=1;k<=Nz;k++) {distdenscarbon[k]=0.0; distdenssurf[k]=0.0;}
	 ...
	 for(k=1;k<=Nz;k++)
	    {
	     ...
	     if(Lattice[i][j][k]==0)
	      {
	       fprintf(outxyz,"F	%lf	%lf	%lf\n",(i-0.5)*a,(j-0.5)*a,(k-0.5)*a);
	       /* Find the distance between the considered accessible site and the closest non accessible site */ 
	       distsurf=sqrt(Lx*Lx+Ly*Ly+Lz*Lz); 
	       /* Diagonal of a rectangular cuboid, larger than the largest distance possible */
	       /* Put as a comment to test diffusion */
	       for(ibis=1;ibis<=Nx;ibis++)
		  {
		   for(jbis=1;jbis<=Ny;jbis++)
		      {
		       for(kbis=1;kbis<=Nz;kbis++)
			  {
			   if(Lattice[ibis][jbis][kbis]==1)
			     {
                    	      dx=fabs(i-ibis)*a; dy=fabs(j-jbis)*a; dz=fabs(k-kbis)*a;
                    	      /* Periodic boundary conditions */
                    	      dx/=Lx; dy/=Ly; dz/=Lz;
                    	      ...
                    	      dist=dx*dx+dy*dy+dz*dz;
                    	      dist=sqrt(dist);
			      if(dist<=distsurf) distsurf=dist;
			     }
			  }
		      }
		  }
	       /* energy as a function of the distance to the surface */
	       idist=1;
	       distmin=fabs(free_eng[1][0]-distsurf);
	       for(l=2;l<=Neng;l++)
		  {
		   dist=fabs(free_eng[l][0]-distsurf);
		   if(dist<=distmin) {distmin=dist; idist=l;}
		  }
	       Esite[i][j][k]=free_eng[idist][1];
	       part_func+=exp(-Esite[i][j][k]/(kB*T));
	       /* Density as a function of distance to the surface */
	       ibin=floor(distsurf/dr);
	       if((ibin>0)&&(ibin<=Nz))
		 {
		  distdenssurf[ibin]+=exp(-Esite[i][j][k]/(kB*T));
		 }
	       /* frequency as a function of the distance to the surface */
               if(strcmp(poreshift,"direct")==0)
	         {
	          idist=1;
	          distmin=fabs(freq[1][0]-distsurf);
	          for(l=2;l<=Nfreq;l++)
		     {
		      dist=fabs(freq[l][0]-distsurf);
		      if(dist<=distmin) {distmin=dist; idist=l;}
		     }
                  wij[i][j][k]=freq[idist][1];
	 	 }
 ...
 for(i=1;i<=Nz;i++)
    {
     fprintf(out,"%e	%lf\n",i*a,distdenssurf[i]*Nf/(part_func)); 
...
void read_coarse()
{
 FILE *in,*inm,*out,*outxyz,*outpores,*outbarr,*outshifts,*outpsd;
 ...
 int ipore,jbefore,jafter,gradient,igrad,jgrad,kgrad,rinumber,ifind,countt;
 double denstot,part_func,poresurf,intrap,slope,gradmin,gradcomp,rdnumber;
 double poresize,meanengz,meanfreqz,meandensz,distmin,sumpb,sumpbm;
 ...
 double aniondens,cationdens,solventdens;
	 ...
	 for(k=1;k<=Nz;k++)
	    {
	     ...
	     if(Lattice[i][j][k]==0)
		   ...
		   /* Choose randomly a pore size on a log normal distribution */
	           /* ATTENTION, from our usual input files the pore size falls approximately in the range from 4 to 15 nms */
		...
		for(l=2;l<=Npores;l++)
		   {
		    if(fabs(poresize-intdens[l][0])<=distmin) {distmin=fabs(poresize-intdens[l][0]); ipore=l;}
                ...
                if(strcmp(surftype,"lognormal")==0) 
	          {
	           ...
	           /* ATTENTION, from our usual input files the pore surface is in nm2 */
                ...
                if(strcmp(densread,"undistinguished")==0)
                  {
                   ...
                   dens[i][j][k][0]=intdens[ipore][1]*poresurf;
                   if(dens[i][j][k][0]<=rhotrshld) goto retake;
		...
		fprintf(outshifts,"%lf	",wij[i][j][k]);
		fprintf(outpsd,"%lf\n",poresize);
                /* From ppm to Hz */
                wij[i][j][k]=(wij[i][j][k]*larmorfreq);
                /* From Hz to rad.s-1 */
        ...
        for(j=1;j<=Ny;j++)
           {
            for(k=1;k<=Nz;k++)
               {
                fprintf(outshifts,"%lf  ",wij[i][j][k]/(2.0*larmorfreq*PI));
               }
           }
       }
    for(k=1;k<=Nz;k++)
	 ...
	 for(k=1;k<=Nz;k++)
		...
		/* Equivalent to ptr=exp(-alpha[i1][j][k][1]/(kB*T)); */
		   ...
		   /* if it stays, velocity equal zero */
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i1][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	          }
	    	if((Esite[i][j][k]>Esite[i1][j][k])&&(Lattice[i1][j][k]==0)) 
	          {
                   ptr=exp(-alpha[i1][j][k][1]/(kB*T));
	    	...
	    	if((Esite[i][j][k]<=Esite[i][j2][k])&&(Lattice[i][j2][k]==0)) 
	      	  {
	       	   ...
	       	   locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j2][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	      	 }
	    	if((Esite[i][j][k]>Esite[i][j2][k])&&(Lattice[i][j2][k]==0)) 
	      	  {
                   ptr=exp(-alpha[i][j2][k][2]/(kB*T));
	    	...
	    	if((Esite[i][j][k]<=Esite[i][j][k1])&&(Lattice[i][j][k1]==0)) 
	      	  {
	       	   ...
	       	   locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j][k1]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	      	  }
	    	if((Esite[i][j][k]>Esite[i][j][k1])&&(Lattice[i][j][k1]==0)) 
	      	  {
                   ptr=exp(-alpha[i][j][k1][5]/(kB*T));
	    	...
	    	if((Esite[i][j][k]<=Esite[i][j][k2])&&(Lattice[i][j][k2]==0)) 
	      	  {
	       	   ...
	       	   locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j][k2]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	      	 }
	    	if((Esite[i][j][k]>Esite[i][j][k2])&&(Lattice[i][j][k2]==0)) 
	      	  {
                   ptr=exp(-alpha[i][j][k2][4]/(kB*T));
		...
		/* Equivalent to ptr=exp(-alpha[i1][j][k][1]/(kB*T)); */
	    	if((Esite[i][j][k]<=Esite[i1][j][k])&&(Lattice[i1][j][k]==0)) 
		  {
		   ...
		   ImG[i][j][k][1]+=dens[i1][j][k][0]*ImG[i1][j][k][0]*ptr;
	           locpfunc+=dens[i1][j][k][0]*ptr;
		   /* What stays */
	    	   for(l=0;l<=dim-1;l++) probv0[i][j][k][l][1]+=dens[i][j][k][0]*probv0[i][j][k][l][0]*(1-exp(-(Esite[i1][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ReG[i][j][k][1]+=dens[i][j][k][0]*ReG[i][j][k][0]*(1-exp(-(Esite[i1][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ImG[i][j][k][1]+=dens[i][j][k][0]*ImG[i][j][k][0]*(1-exp(-(Esite[i1][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i1][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
		  }
	    	if((Esite[i][j][k]>Esite[i1][j][k])&&(Lattice[i1][j][k]==0)) 
	          {
	    	...
	    	if((Esite[i][j][k]<=Esite[i2][j][k])&&(Lattice[i2][j][k]==0)) 
		  {
	           ...
	           locpfunc+=dens[i2][j][k][0]*ptr;
	           for(l=0;l<=dim-1;l++) probv0[i][j][k][l][1]+=dens[i][j][k][0]*probv0[i][j][k][l][0]*(1-exp(-(Esite[i2][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ReG[i][j][k][1]+=dens[i][j][k][0]*ReG[i][j][k][0]*(1-exp(-(Esite[i2][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ImG[i][j][k][1]+=dens[i][j][k][0]*ImG[i][j][k][0]*(1-exp(-(Esite[i2][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i2][j][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
		  }
	    	if((Esite[i][j][k]>Esite[i2][j][k])&&(Lattice[i2][j][k]==0)) 
	          {
	    	...
	    	if((Esite[i][j][k]<=Esite[i][j1][k])&&(Lattice[i][j1][k]==0)) 
		  {
	           ...
	           locpfunc+=dens[i][j1][k][0]*ptr;
	           for(l=0;l<=dim-1;l++) probv0[i][j][k][l][1]+=dens[i][j][k][0]*probv0[i][j][k][l][0]*(1-exp(-(Esite[i][j1][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ReG[i][j][k][1]+=dens[i][j][k][0]*ReG[i][j][k][0]*(1-exp(-(Esite[i][j1][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ImG[i][j][k][1]+=dens[i][j][k][0]*ImG[i][j][k][0]*(1-exp(-(Esite[i][j1][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j1][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
		  }
	   	if((Esite[i][j][k]>Esite[i][j1][k])&&(Lattice[i][j1][k]==0)) 
	          {
	   	...
	   	if((Esite[i][j][k]<=Esite[i][j2][k])&&(Lattice[i][j2][k]==0)) 
		  {
	           ...
	           locpfunc+=dens[i][j2][k][0]*ptr;
	           for(l=0;l<=dim-1;l++) probv0[i][j][k][l][1]+=dens[i][j][k][0]*probv0[i][j][k][l][0]*(1-exp(-(Esite[i][j2][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ReG[i][j][k][1]+=dens[i][j][k][0]*ReG[i][j][k][0]*(1-exp(-(Esite[i][j2][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ImG[i][j][k][1]+=dens[i][j][k][0]*ImG[i][j][k][0]*(1-exp(-(Esite[i][j2][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j2][k]-Esite[i][j][k])/(kB*T)))*(1-ptr);
		  }
	   	if((Esite[i][j][k]>Esite[i][j2][k])&&(Lattice[i][j2][k]==0)) 
	          {
	   	...
	   	if((Esite[i][j][k]<=Esite[i][j][k1])&&(Lattice[i][j][k1]==0)) 
		  {
	           ...
	           locpfunc+=dens[i][j][k1][0]*ptr;
	           for(l=0;l<=dim-1;l++) probv0[i][j][k][l][1]+=dens[i][j][k][0]*probv0[i][j][k][l][0]*(1-exp(-(Esite[i][j][k1]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ReG[i][j][k][1]+=dens[i][j][k][0]*ReG[i][j][k][0]*(1-exp(-(Esite[i][j][k1]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ImG[i][j][k][1]+=dens[i][j][k][0]*ImG[i][j][k][0]*(1-exp(-(Esite[i][j][k1]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j][k1]-Esite[i][j][k])/(kB*T)))*(1-ptr);
		  }
	   	if((Esite[i][j][k]>Esite[i][j][k1])&&(Lattice[i][j][k1]==0)) 
	          {
	   	...
	   	if((Esite[i][j][k]<=Esite[i][j][k2])&&(Lattice[i][j][k2]==0)) 
		  {
	           ...
	           locpfunc+=dens[i][j][k2][0]*ptr;
	           for(l=0;l<=dim-1;l++) probv0[i][j][k][l][1]+=dens[i][j][k][0]*probv0[i][j][k][l][0]*(1-exp(-(Esite[i][j][k2]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ReG[i][j][k][1]+=dens[i][j][k][0]*ReG[i][j][k][0]*(1-exp(-(Esite[i][j][k2]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           ImG[i][j][k][1]+=dens[i][j][k][0]*ImG[i][j][k][0]*(1-exp(-(Esite[i][j][k2]-Esite[i][j][k])/(kB*T)))*(1-ptr);
	           locpfunc+=dens[i][j][k][0]*(1-exp(-(Esite[i][j][k2]-Esite[i][j][k])/(kB*T)))*(1-ptr);
		  }
	   	if((Esite[i][j][k]>Esite[i][j][k2])&&(Lattice[i][j][k2]==0)) 
	          {
